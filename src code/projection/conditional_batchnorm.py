...
class ConditionalBatchNorm2d(nn.BatchNorm2d):
    ...
    def forward(self, input, weight, bias, **kwargs):
        ...
        '''# [bs,num_classes,num_features,1,1]
        weight = weight.unsqueeze(-1).unsqueeze(-1) # [bs,num_classes,num_features]
        weight=torch.sum(weight,dim=1) # [bs,num_features]
        weight=weight.expand(size)  # [bs,num_features,num_width,num_width]
...
class CategoricalConditionalBatchNorm2d(ConditionalBatchNorm2d):
    ...
    def __init__(self, num_classes, num_features, eps=1e-5, momentum=0.1,
        ...
        # use linear
        self.weights = nn.Embedding(num_classes+1, num_features)
        self.l_w=nn.Linear(num_features*3,num_features)
        
        self.biases = nn.Embedding(num_classes+1, num_features) # [bs, dim_in, num_features]
        self.l_b=nn.Linear(num_features*3,num_features)
    ...
    def forward(self, input, c, **kwargs):
        ...
        # c=[bs,3] contains label id
        bs=c.shape[0]
        '''
        # use linear
        weight=self.l_w(self.weights(c).view(bs,-1))
        bias=self.l_b(self.biases(c).view(bs,-1))'''

        weight=torch.sum(self.weights(c),dim=1).squeeze(dim=1)
        bias=torch.sum(self.biases(c),dim=1).squeeze(dim=1)
